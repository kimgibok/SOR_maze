...
void Engine::GenerateMaze()
{
	...
	char generated[11][11]; //Generate a 11 by 11 maze
	...
	while (!path.empty())
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (cell2.first >= 0 && cell2.first <= 10 && cell2.second >= 0 && cell2.second <= 10)
			{
		...
		if (frontier.empty())
		{
			...
			while (!path.empty() && !found)
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					//Make sure that the cell we are going to is within bounds
					if (c2.first >= 0 && c2.first <= 10 && c2.second >= 0 && c2.second <= 10)
					{
						//Check to see if that cell has been visited yet
	...
	generated[9][5] = '@'; //(9, 5) is where the player starts
	cameraRotation = 0.0f; //Set camera rotation to be 0 radians
	generated[1][5] = 'E'; //(1, 5) is where the maze ends
